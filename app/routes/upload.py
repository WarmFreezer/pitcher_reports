...
from app.services import report, hitter_report, report_lab_generator, file_validator
...
def upload_file():
    ...
    try:
        ...
        stat_type = request.form.get('statType', 'pitcher')
        ...
        if stat_type == 'pitcher':
            for pitcher_id in source_df['PitcherId'].unique():
                ...
                gc.collect()
        elif stat_type == 'hitter':
            for hitter_id in source_df['BatterId'].unique():
                try:
                    hitter_team = source_df.loc[source_df['BatterId'] == hitter_id, 'BatterTeam'].iloc[0]
                    is_own = hitter_team == current_user.school.trackman_id
                    if target == 'opponent' and is_own:
                        continue
                    if target != 'opponent' and not is_own:
                        continue
                    ...
                    if current_user.school.is_active:
                        for theme in ('light', 'dark'):
                            hitter_report.hitter_heat_map_by_pitcher_side()
                            hitter_report.hitter_spray_chart_by_pitcher_side()
                            continue

                    # Build hitter stat tables
                    table_data = build_hitter_table(source_df, hitter_id)
                    if not table_data or len(table_data) < 2 or table_data[1] is None:
                        raise ValueError(f'Failed to build table data for hitter ID {hitter_id}')

                    # Auto-add hitters found in the game file but missing from the roster
                    if not roster.empty and hitter_id not in roster['Trackman ID'].values and target != 'opponent':
                        last_name, first_name = source_df.loc[source_df['BatterId'] == hitter_id, 'Batter'].iloc[0].split(', ', 1)
                        roster = pd.concat([roster, pd.DataFrame([{'Trackman ID': hitter_id, 'First Name': first_name, 'Last Name': last_name}])])

                    
                except Exception as e:
                    print(f"Error processing hitter ID {hitter_id}: {e}")
                    flash_toast(f"Error processing hitter {hitter_id}: {str(e)}", type='error')
                    continue
        ...
        merged_pdf_path = os.path.join(school_output_folder, f'{current_user.id}_merged_{stat_type}_reports.pdf')
        ...
        return jsonify({
            ...
            'reports': reports,
            'merged_pdf_url': f'/storage/schools/{current_user.school.slug}/reports/{current_user.id}_merged_{stat_type}_reports.pdf',
